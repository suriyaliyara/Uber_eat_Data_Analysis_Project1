...
def uber_eats_master_cleaner(df_rest):
    ...
    if 'name' in df_rest.columns:
        ...
        # Second: Convert the whole column to float numbers
        #df_rest['rate'] = pd.to_numeric(df_rest['rate'], errors='coerce')
    
        # Third: Calculate the mean of the column (Pandas ignores None/NaN automatically)
        #column_mean = df_rest['rate'].mean()
    
        # Fourth: Fill the empty "NEW" slots with that mean
        #df_rest['rate'] = df_rest['rate'].fillna(round(column_mean, 1))
...
#df = pd.read_csv(r"C:\Users\Niruban\Documents\Project1\Uber_Eats_data.csv")
...
print(df_cleaned.head(200)) #23158 rows × 13 columns
...
def uber_orders_cleaner(df_orders):
    ...
    if 'name' in df_orders.columns:
        #df2['name'] = df2['name'].apply(rule_1_encoding).apply(rule_2_names)
...
# --- RUNNING THE PIPELINE ---
#df_orders = pd.read_json(r"C:\Users\Niruban\Documents\Project1\orders.json") #totaol count: 25000 rows × 6 columns
...
#display(df2_cleaned.dtypes)#count:25000 rows × 6 columns
...
# 2. Push your cleaned DataFrame to the table 'restaurants'
#df_cleaned.to_sql('restaurants', conn, if_exists='replace', index=False)
#df1_cleaned.to_sql('orders', conn, if_exists='replace', index=False)
